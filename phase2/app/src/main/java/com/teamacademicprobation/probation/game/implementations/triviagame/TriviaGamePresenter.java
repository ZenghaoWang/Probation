...
class TriviaGamePresenter {
	 ...
	 * screen if there are no more questions, and updates the elements on the sckreen otherwise.
	 */
	void updateView() {
		model.getRandomQuestion();
		if (model.isCompleted()) {
			view.disableInput();
			view.goToScoreScreen(model.generateScoreMessage(),model.getPlayerID(), TriviaGameModel.GAMEID);
		} else {
